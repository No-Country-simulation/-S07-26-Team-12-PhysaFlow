...
El script `npm run seed` permite poblar la base con un set de datos consistente (leads, cálculos asociados y sin asociar, y resultados compartidos), útil para desarrollo y demos.
...
Representa un potencial cliente/usuario que solicitó o generó un cálculo.

| Campo | Tipo | Restricciones |
|---|---|---|
| `id` | UUID | PK, autogenerado (`UUIDV4`) |
| `name` | STRING | Opcional |
| `email` | STRING | Obligatorio, único, validado como email |
| `source` | ENUM(`direct`, `shared`) | Obligatorio, default `direct` |
| `status` | ENUM(`new`, `contacted`, `converted`) | Obligatorio, default `new` |
| `createdAt` / `updatedAt` | DATE | Automáticos (`timestamps: true`) |

### `Calculation` (tabla: `calculations`)

Representa un cálculo de capacidad varada realizado sobre una instalación (data center).

| Campo | Tipo | Restricciones |
|---|---|---|
| `id` | UUID | PK, autogenerado |
| `lead_id` | UUID | Opcional (puede ser `NULL` si el cálculo se hizo sin lead asociado) |
| `facility_size_mw` | FLOAT | Obligatorio |
| `utilization_percentage` | FLOAT | Obligatorio, default `0` |
| `cooling_type` | ARRAY(STRING) | Obligatorio; valida que tenga al menos un valor y que todos pertenezcan a `["air", "liquid", "immersion"]` |
| `stranded_capacity_percent` | FLOAT | Calculado, default `0` |
| `stranded_capacity_mw` | FLOAT | Calculado, default `0` |
| `annual_loss_min` | FLOAT | Calculado, default `0` |
| `annual_loss_max` | FLOAT | Calculado, default `0` |
| `formula_version` | STRING | Opcional, identifica qué versión de la fórmula generó el resultado |
| `createdAt` / `updatedAt` | DATE | Automáticos |

> Nota: aunque el modelo permite múltiples valores en `cooling_type`, la fórmula actual (`calculationFormula.js`) solo toma el **primer elemento del array** para calcular el factor de enfriamiento. Está señalado como comportamiento actual, sujeto a evolución.

### `SharedResult` (tabla: `shared_results`)

Representa un resultado de cálculo compartido públicamente mediante un token.

| Campo | Tipo | Restricciones |
|---|---|---|
| `id` | UUID | PK, autogenerado |
| `calculation_id` | UUID | Obligatorio (FK a `Calculation`, definida vía asociación en `index.models.js`) |
| `view_count` | INTEGER | Default `0` |
| `share_token` | STRING | Obligatorio, único |
| `expiresAt` | DATE | Default: fecha actual + 7 días |
| `createdAt` / `updatedAt` | DATE | Automáticos |
...
Todas las rutas están montadas bajo el prefijo **`/api`** (definido en `app.js`: `app.use("/api", indexRouter)`), y agrupadas en `src/routes/index.routes.js`:
...
|---|---|---|
| `GET` | `/health` | Healthcheck simple, responde `200` si el servidor está corriendo |
...
|---|---|---|---|
| `POST` | `/api/calculations/calculate` | ✅ Implementado | Recibe `facility_size_mw`, `utilization_percentage` y `cooling_type`, valida el input (DTO), calcula la capacidad varada y la pérdida anual estimada, persiste el `Calculation` y lo devuelve |
| `GET` | `/api/calculations/` | 🔜 Pendiente | Listar cálculos |
| `GET` | `/api/calculations/:id` | 🔜 Pendiente | Obtener un cálculo por id |
| `PUT` | `/api/calculations/:id` | 🔜 Pendiente | Actualizar un cálculo |
| `DELETE` | `/api/calculations/:id` | 🔜 Pendiente | Eliminar un cálculo |

**Ejemplo de request a `POST /api/calculations/calculate`:**
...
Posibles errores: `400` (input inválido), `422` (error de validación de Sequelize/DTO), `500` (error interno).

### `/api/leads`

| Método | Endpoint | Estado |
|---|---|---|
| `POST` / `GET` / `GET /:id` / `PUT /:id` / `DELETE /:id` | `/api/leads/...` | 🔜 Pendiente — el router existe pero todos los endpoints están comentados a la espera de implementación |

### `/api/shared-results`

| Método | Endpoint | Estado |
|---|---|---|
| `POST` / `GET` / `GET /:token` / `DELETE /:id` | `/api/shared-results/...` | 🔜 Pendiente — el router existe pero todos los endpoints están comentados a la espera de implementación |
