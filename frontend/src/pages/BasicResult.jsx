...
export default function BasicResult() {
  ...
  const strandedCapacity = result?.data?.stranded_capacity_mw ?? 38.4;

  const minLoss = result.data.annual_loss_min;
  const maxLoss = result.data.annual_loss_max;
  const cooling = result.data.cooling_type;
    ...
    <PageConainer>
      <div className={`flex flex-col items-center justify-center p-4`}>
        <p className="data-medium text-gold-darkest">
          TU RESULTADO · 
          {result.data.facility_size_mw} MW · 
          {result.data.utilization_percentage}% UTIL · 
        ...
        <div className={`flex ${isMobile ? "flex-col": "flex-row"}`}>
          <div className="p-4 flex flex-col justify-center iitems-center">
            <h1 className="text-center display-data-big">{result.data.stranded_capacity_percent * 100}%</h1>
            <p className="text-center data-small-green">CAPACIDAD ESTANCADA</p>
          </div>
          <div className="p-4 flex flex-col justify-center iitems-center">
            <h1 className="text-center display-data-big">{Math.round(result.data.stranded_capacity_mw *10)/10} MW</h1>
